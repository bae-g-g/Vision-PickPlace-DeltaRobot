...
void ParseLongs(String &str, long *out, int count);
...
Task tGetCommand(10, TASK_FOREVER, GetCommand, &scheduler,
                 false); // 태스크 객체 생성
Task tHandleCommand(10, TASK_ONCE, HandleCommand, &scheduler,
                    false);                         // 태스크 객체 생성
Task tRun(0, TASK_FOREVER, run, &scheduler, false); // 태스크 객체 생성
Task tDownmove(1, TASK_FOREVER, downmove, &scheduler,
               false);                                     // 태스크 객체 생성
...
void setup() {
  ...
  for (int i = 0; i < STEPPERS_NUM; i++) {
    stepper[i].setMaxSpeed(
        20000.0); // 1초에 1000스텝(약 1/200도 × 1000 = 5도/초)
  ...
  pinMode(VACUUM_INTERRUPT_PIN, INPUT_PULLUP); // 인터럽트 핀 설정
...
void ParseLongs(String &str, long *out, int count) {
  ...
  for (int i = 0; i < count; i++) {
    while (idx < str.length() && !isDigit(str[idx]) && str[idx] != '-' &&
           str[idx] != '+') {
      idx++;
    }
    int next_idx = idx;
    while (next_idx < str.length() &&
           (isDigit(str[next_idx]) || str[next_idx] == '-' ||
            str[next_idx] == '+')) {
      next_idx++;
    }
    if (idx < str.length()) {
      out[i] = str.substring(idx, next_idx).toInt();
    }
    idx = next_idx + 1;
  }
}
...
void GetCommand() {
  if (Serial.available() && command_flag == 0) {
    ...
    if (command[0] == 'M' || command[0] == 'R' || command[0] == 'G' ||
        command[0] == 'S' || command[0] == 'D') { // 명령어가 M, R, G, S, D일 때
...
void HandleCommand() {
  if (command[0] == 'M') {
    long position[3];
    ParseLongs(command, position, 3);
    command_queue.push(
        COMMAND_DATA(command[0], position[0], position[1], position[2]));
  } else if (command[0] == 'S') {
    // 속도 변경 명령어 처리
    long speed[3];
    ParseLongs(command, speed, 3);
...
void run() {
  if (stepper[0].distanceToGo() || stepper[1].distanceToGo() ||
  ...
  if (command_data.command == 'M') {
    ...
    for (int i = 0; i < STEPPERS_NUM; i++) {
      ...
      stepper[i].setAcceleration(command_data.data[i] /
                                 2.0); // 가속도는 속도의 절반으로 설정
    }
    ...
    while (1) {
      ...
      if (digitalRead(ENDSTOP_PIN3))
        stepper[2].runSpeed();
      if (!digitalRead(ENDSTOP_PIN1) && !digitalRead(ENDSTOP_PIN2) &&
          !digitalRead(ENDSTOP_PIN3))
        break;
    }
    ...
    if (command_data.data[0] == 1) {
      ...
      delay(10);
      attachInterrupt(digitalPinToInterrupt(VACUUM_INTERRUPT_PIN),
                      VacuumInterrupt, FALLING);
...
void SpeedSynchronization(long position[]) {
  ...
  for (int i = 0; i < 3; i++) {
    if (distance[i] > 0) {
      stepper[i].setMaxSpeed(MAX_SPEED * (float)distance[i] / (float)max);
      stepper[i].setAcceleration(MAX_ACCELERATION * (float)distance[i] /
                                 (float)max);
    }
  }
}
...
void VacuumInterrupt() {
  for (int i = 0; i < STEPPERS_NUM; i++) {
    stepper[i].moveTo(
        stepper[i].currentPosition()); // 현재 위치를 목표 포지션으로 설정
  ...
  command_queue.push_front(
      COMMAND_DATA('M', stepper[0].currentPosition() + 1000,
                   stepper[1].currentPosition() + 1000,
                   stepper[2].currentPosition() + 1000)); // 목표 포지션 설정
}
